...
> 구현 중 변경(반영 완료):
> - `add_event_choice.dart`가 `day_detail_view.dart`에서도 쓰이고 있었다. 사용자 승인에 따라
>   상세화면 '일정' 섹션의 '추가'는 `showEventEditor`(일정 편집) 직행으로 바꾸고(축일 선택 제거,
>   축일은 메인 스피드다이얼로), `add_event_choice.dart`는 삭제했다.
> - `SpeedDialFab`은 `required EdgeInsets padding`을 받아 자체적으로 `Padding + Align(bottomRight)`로
>   버튼 컬럼을 배치한다. `calendar_page`는 `Positioned.fill(child: _addEventFab(s, padding: ...))`로
>   tight 전체화면 제약을 넘긴다(외부 Align 래핑 금지 - 스크림이 화면 전체를 덮게 하기 위함).
> - 오래된 위젯 테스트(`the add-event FAB pushes the editor screen`)를 스피드다이얼 흐름으로 갱신하고,
>   스크림 전체화면 커버 회귀 테스트를 추가했다.

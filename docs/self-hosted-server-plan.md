...
Android에서는 Google Drive `appDataFolder`를 사용한다.

- 앱 전용 숨김 폴더에 개인 일정/카테고리 백업 JSON을 저장한다.
- `drive.appdata` OAuth scope가 필요하다.
- 앱만 접근할 수 있고 일반 Drive UI에는 보이지 않는다.
- 앱이 직접 업로드/다운로드 타이밍을 제어할 수 있다.
- 사용자가 일정을 변경하면 즉시 업로드하고, 앱 시작 시 가능한 경우 즉시 다운로드해 복원한다.
- Google Sign-In/OAuth 동의가 필요하므로 개인정보 처리방침과 앱 내 안내를 함께 유지한다.

Android Auto Backup은 즉시 동기화 시점을 앱이 제어하기 어렵기 때문에 주 백업 방식으로 사용하지 않는다.
...
- Android: Google Drive `appDataFolder` 백업/복원 구현
- 앱 삭제 후 재설치 복원 테스트
...
Android는 Google Sign-In으로 `drive.appdata` 권한을 받은 뒤 Google Drive `appDataFolder`에
`personalDataSnapshotV1.json`을 저장하고 다시 불러온다.
...
22단계의 Google/Android 작업에서는 Android Auto Backup 대신 Google Drive 앱 전용 저장소를 직접
사용하도록 수정했다. 앱은 일반 Google Drive 파일을 조회하지 않고, 앱 전용 백업 파일만 생성/수정/조회한다.
일정 또는 카테고리 변경 시 백업 JSON을 바로 업로드하고, 앱 시작 시 이전 인증 상태가 남아 있으면 조용히
백업 JSON을 다운로드해 복원한다.

22단계 Android 테스트 시나리오:

1. 실제 Android 기기에 앱 설치
2. `2026년 7월 19일` 개인 일정 추가
3. Google 계정 선택 및 Drive 앱 전용 저장소 권한 동의
4. debug log에서 `[KCC backup] Cloud personal-data backup saved` 확인
5. 앱 삭제
6. 앱 재설치
7. 앱 실행 후 `[KCC backup] Restored cloud personal-data snapshot` 확인
8. `2026년 7월 19일` 일정이 자동 복원되어 보이는지 확인

이후 설정 화면에는 `Google 계정 연결`, `지금 백업`, `지금 복원`, `연결 해제` 버튼을 추가해 사용자가
직접 동기화 상태를 확인하고 제어할 수 있게 하는 것이 좋다.

### 5단계: 확장

- iOS CloudKit Private Database 전환 검토
- Android Google Drive 동기화 설정 UI 추가
- 여러 기기 충돌 처리 개선
- 푸시 알림 연동 검토
...
4. Android Google Drive 앱 전용 백업/복원 검증

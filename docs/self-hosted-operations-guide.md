...
`업데이트 정책` 화면에서는 iOS/Android 공통 업데이트 안내 방식을 관리한다. 정책은 `안내 없음`,
`권장 업데이트`, `강제 업데이트` 중 하나다.

- 안내 없음: 앱에 업데이트 팝업을 표시하지 않음
- 권장 업데이트: 앱 실행 시 업데이트 안내를 띄우되 `다음에`로 닫고 앱을 계속 사용할 수 있음
- 강제 업데이트: 앱 실행 시 업데이트 안내를 띄우고 `업데이트` 버튼만 제공

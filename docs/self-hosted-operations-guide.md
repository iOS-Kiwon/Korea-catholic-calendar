...
- `scripts/server-*.sh`: 빌드, 시작, 중지, 재시작, 상태 확인, 로그, 백업, 복구 스크립트
...
`업데이트 정책` 화면에서는 iOS와 Android 각각의 업데이트 기준 버전과 안내 방식을 관리한다.
기준 버전은 `1.2.3` 형식으로 입력하고, 안내 방식은 `안내 없음`, `권장 업데이트`, `강제 업데이트`
중 하나다.

- 안내 없음: 앱에 업데이트 팝업을 표시하지 않음
- 권장 업데이트: 현재 앱 버전이 기준 버전보다 낮으면 업데이트 안내를 띄우되 `다음에`로 닫고 앱을 계속 사용할 수 있음
- 강제 업데이트: 현재 앱 버전이 기준 버전보다 낮으면 업데이트 안내를 띄우고 `업데이트` 버튼만 제공

강제 업데이트 팝업은 타이틀, 메시지, `업데이트` 버튼 1개로 구성한다. 권장 업데이트 팝업은 타이틀,
메시지, `다음에`, `업데이트` 버튼 2개로 구성한다. 버튼 문구와 스토어 URL은 앱에 고정한다.
iOS는 Apple 앱 ID `6791044471`, Android는 번들 ID `com.sidore.catholiccalendar`로 각 스토어를
연다. 앱은 실행 시 `GET /kcc/v1/app/version?platform=ios&version=1.0.0` API를 호출해 이 정책을
...
현재 백업 파일은 `BACKUP_DIR`에 `db-YYYYMMDD-HHMMSS.sql.gz` 형식으로 저장된다. `BACKUP_DIR`를
따로 설정하지 않으면 repo 아래 `backups/`를 사용한다.
...
자동 백업은 launchd로 매일 새벽에 실행하도록 설정할 수 있다.

1. 예시 파일을 사용자 LaunchAgents에 복사한다.

```bash
cp ops/launchd/com.sidore.catholic-calendar-backup.plist.example \
  ~/Library/LaunchAgents/com.sidore.catholic-calendar-backup.plist
```

2. 파일 안의 경로를 실제 Mac mini repo 위치로 바꾼다.

```text
/Users/wishingnote/Work/Korea-catholic-calendar/scripts/server-backup.sh
/Users/wishingnote/Work/Korea-catholic-calendar
```

3. 문법을 확인하고 등록한다.

```bash
plutil -lint ~/Library/LaunchAgents/com.sidore.catholic-calendar-backup.plist
launchctl load ~/Library/LaunchAgents/com.sidore.catholic-calendar-backup.plist
```

4. 즉시 한 번 실행해본다.

```bash
launchctl start com.sidore.catholic-calendar-backup
ls -lh backups
```

운영 권장사항:

- 백업 파일은 Mac mini 내부 디스크에만 두지 말고 외장 디스크, NAS, iCloud Drive, 별도 클라우드 중
  하나에 추가 복사한다.
- 백업 파일에는 운영 데이터가 들어 있으므로 공개 저장소에 올리지 않는다.
- 최소 월 1회는 실제 복구 테스트를 한다.
- 오래된 백업을 자동 삭제하는 보존 정책은 데이터가 안정화된 뒤 추가한다. 예: 30일 보관.

## DB 복구

복구는 기존 DB를 지우고 백업 파일 내용으로 되돌리는 작업이다. 실수로 실행하면 현재 백오피스 수정값과
캐시가 사라질 수 있으므로 반드시 필요한 경우에만 진행한다.

복구 전 확인:

1. 현재 장애가 DB 문제인지 로그로 확인한다.
2. 가능하면 현재 상태도 한 번 더 백업한다.
3. 복구할 백업 파일 날짜를 확인한다.
4. 백오피스에서 작업 중인 사람이 없는지 확인한다.
...
```bash
./scripts/server-restore.sh backups/db-YYYYMMDD-HHMMSS.sql.gz --yes
```
...
`admin.sidore.org`는 Cloudflare Access가 정상이라면 `302`로 Access 로그인 화면으로 이동하는 것이
정상이다.

## 장애 시 확인 순서

앱에서 달력이 안 보이거나 백오피스가 열리지 않을 때는 아래 순서로 좁혀간다.
...
```bash
dig api.sidore.org
curl -v https://api.sidore.org/kcc/v1/health
dig admin.sidore.org
curl -I https://admin.sidore.org/kcc
```

자주 보는 증상:

- `curl: Could not resolve host`: Mac mini DNS 문제일 가능성이 높다. Ethernet DNS와 macOS DNS 캐시를
  확인한다.
- Cloudflare `1033`: DNS는 맞지만 Tunnel이 Cloudflare에 연결되어 있지 않다.
- API 헬스체크 실패: `api` 로그와 `db` 로그를 먼저 본다.
- 백오피스 로그인 화면은 뜨지만 데이터가 안 보임: 백오피스에서 API 내부 주소
  `API_INTERNAL_BASE_URL=http://api:8080/kcc/v1` 설정을 확인한다.

## Cloudflare Rate Limit/WAF 권장 설정

`api.sidore.org`는 앱 사용자가 접근해야 하므로 Cloudflare Access를 걸지 않는다. 대신 공개 API로서
기본 방어 정책을 둔다.

권장 규칙:

- `admin.sidore.org/kcc*`: Cloudflare Access 필수
- `api.sidore.org/kcc/v1/health`: Rate Limit에서 제외하거나 매우 느슨하게 적용
- `api.sidore.org/kcc/v1/calendar/*`: IP 기준 분당 60회 또는 5분 300회 초과 시 Managed Challenge
- `api.sidore.org/kcc/v1/app/version`: IP 기준 분당 60회 또는 5분 300회 초과 시 Managed Challenge
- `api.sidore.org`의 `POST`, `PUT`, `PATCH`, `DELETE` 요청은 현재 공개 API에 필요하지 않으므로
  WAF에서 Block 또는 Managed Challenge
- Cloudflare Managed Ruleset은 켜두고, 오탐이 있으면 해당 규칙만 예외 처리

운영 초반에는 바로 차단보다 `Managed Challenge`로 시작하는 편이 안전하다. 실제 앱 사용자에게 문제가
없다는 것을 확인한 뒤 공격성 트래픽만 `Block`으로 올린다.

...
`앱 버전 관리` 화면에서는 iOS/Android별 최소 허용 버전, 최신 권장 버전, 스토어 URL, 업데이트
팝업 문구를 관리한다. 앱 버전은 현재 앱 표시 형식과 동일하게 `1.2.3` 형태만 허용한다.

- 현재 앱 버전 < 최소 허용 버전: 강제 업데이트
- 현재 앱 버전 < 최신 권장 버전: 권장 업데이트
- 현재 앱 버전 >= 최신 권장 버전: 업데이트 안내 없음

강제 업데이트 팝업은 타이틀, 메시지, `업데이트` 버튼 1개로 구성한다. 권장 업데이트 팝업은 타이틀,
메시지, `다음에`, `업데이트` 버튼 2개로 구성한다. 앱은 다음 단계에서
`GET /kcc/v1/app/version?platform=ios&version=1.0.0` API를 호출해 이 정책을 적용한다.

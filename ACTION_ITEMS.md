...
- **일정 = 카테고리 선택 방식** (제목 직접 입력 없음): 사용자가 자기 카테고리를 만들고, 일정 추가 시 **카테고리를 클릭**해 등록. 메모만 부가 설명으로 직접 입력.
  - 카테고리: **이름 + 색상**(아이콘 없음). 관리 화면에서 **추가/편집/삭제/순서변경**.
  - **관리 진입**: 일정 추가/수정 화면의 **설정(⚙) 버튼**에서 진입(+ 편집 시트 안 '추가' 칩으로 즉석 생성).
  - **첫 실행 시 기본 카테고리 시드**: 본당 행사·교구 행사·기도·전례·교리 (모두 편집/삭제 가능).
  - 카테고리 **이름/색 변경 → 기존 일정에도 반영**. **삭제는 사용 중이면 차단**(해당 일정을 먼저 정리해야 삭제 가능, 삭제는 카테고리만 제거).
- 브랜치: `feat/local-events` (커밋 완료). 검증: `flutter analyze` 클린, `flutter test` 28개 통과, `flutter build web` 성공.

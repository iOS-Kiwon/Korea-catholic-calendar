...
- **확정된 조건 (전부 AND, 하나라도 어긋나면 요청 안 함)**: 앱 설치 후 7일 경과 + 앱 실행(세션) 8회 이상 + 저장된 일정 5개 이상 + 아직 요청한 적 없음(**설치당 영구 1회**) + 서버 킬스위치 켜짐.
- **노출 시점**: **일정을 저장한 직후, 오직 이때만.** 앱 시작 시에는 띄우지 않습니다. "앱을 열어보기만 한 사용자에게 리뷰를 청하는 건 과하다"는 결정에 따른 것입니다.
- **"실행 1회"의 정의**: 콜드 스타트 + 백그라운드에 30분 이상 있다가 복귀. iOS는 앱을 잘 안 죽여서 콜드 스타트만 세면 8회 채우는 데 수 주가 걸릴 수 있어 세션 기준을 택했습니다.
- **경과일 기준**: 자체 기록이 아니라 **실제 앱 설치 시각**(`PackageInfo.installTime` - Android는 OS의 `firstInstallTime`, iOS는 Documents 디렉터리 생성일).
- ⚠️ **실행 횟수만은 설치 시점부터 소급이 불가능합니다.** OS가 앱 실행 횟수를 기록해주지 않아 앱이 직접 세야 하므로, 이 기능이 담긴 버전을 **배포한 뒤부터** 카운트됩니다. 기존 사용자는 설치 경과일·일정 개수는 즉시 통과하고 실행 횟수만 새로 쌓게 됩니다.
...
- **되돌릴 수 없는 선택**: "최초 1회"는 소진하면 회복이 없습니다. 게다가 iOS는 365일 3회, Android는 비공개 쿼터가 있어 시스템이 프롬프트를 조용히 삼켜도 앱은 알 수 없습니다. 그래서 서버 킬스위치를 둡니다.

## ⏳ 남은 당신의 몫

- [ ] **리뷰 요청 기준치 최종 승인** - 설치 7일 / 실행 8회 / 일정 5개. AND로 묶여 있어 조건이 빡빡한 편이라, 노출 모수가 생각보다 작을 수 있습니다. 구현 착수 전에 숫자를 조정하고 싶으면 알려주세요.
...
- [ ] **리뷰창 실기기 검증** - 임계값을 낮춘 디버그 빌드로 실제 노출 확인. iOS는 TestFlight/디버그 빌드에서만 항상 뜨고 프로덕션에서는 쿼터가 적용됩니다.

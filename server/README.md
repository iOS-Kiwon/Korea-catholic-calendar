...
불변이라, 한 달당 CBCK를 사실상 **1회만** 호출하고 이후 요청은 KV 캐시에서
...
- 매시간 cron으로 **1개월만** 프리워밍. `meta:last_cached_month`를 기준으로 마지막
  저장 월 다음 달부터 순차적으로 채움.
- 발행 데이터는 만료 없이 KV에 저장. 미발행/빈 결과만 1일 네거티브 캐시 후 재시도.
...
> ⚠️ 과거 여러 해를 **한꺼번에 백필하지 마세요.** cron이 매시간 1개월씩 천천히 채우고,
> 사용자가 방문하는 달은 lazy로 채워집니다. 대량 일괄 크롤링은 차단·차질 위험이 있습니다.
